  ...
  if (!post) {
    return (
      <div className="min-h-[50vh] flex flex-col items-center justify-center">
        ...
        <Link href="/blog" className="text-yellow-600 hover:underline">Return to Map</Link>
      </div>
    );
  }
  ...
  return (
      <div className="w-full max-w-5xl mx-auto bg-white dark:bg-slate-900 shadow-2xl relative flex flex-col min-h-screen">
          ...
          <div className="relative h-64 md:h-80 shrink-0">
             {post.image && <img src={post.image} className="w-full h-full object-cover" alt={post.title} />}
             <Link
               href="/blog"
             ...
             <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 bg-gradient-to-t from-black/90 to-transparent pt-20">
                ...
                <div className="flex flex-wrap gap-4 text-slate-300 text-sm font-mono">
                  ...
                  <span className="text-green-400 font-bold bg-green-400/10 px-2 rounded border border-green-400/20">{post.xp} XP</span>
                </div>
             </div>
          </div>
          ...
          <div className="flex-grow p-6 md:p-10 bg-white dark:bg-[#0b1221]">
             <div className="max-w-3xl mx-auto">
               <div className="prose prose-slate dark:prose-invert max-w-none prose-lg prose-headings:font-pixel-bold-cn prose-a:text-yellow-600 dark:prose-a:text-yellow-400 hover:prose-a:text-yellow-500">
                  <MDXRemote source={post.content} options={{ parseFrontmatter: true }} components={mdxComponents} />
               </div>
               
               <div className="mt-12 pt-8 border-t border-slate-200 dark:border-slate-800 flex justify-center">
                  <Link href="/blog" className="px-8 py-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full font-pixel-bold-cn transition-colors text-slate-700 dark:text-slate-300 flex items-center gap-2 group">
